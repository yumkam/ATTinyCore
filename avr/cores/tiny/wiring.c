...
static void initToneTimerInternal(void);
  ...
  unsigned long micros()
  {
    ...
    return ((m << 8) + t) * MillisTimer_Prescale_Value * (1000000L/F_CPU);
      ...
      m = (m << 8) + t;
      return (m<<1) + ((m + ((m + (m >> 1)) >> 2)) >> 1); // multiply by 2.6875
    #elif (MillisTimer_Prescale_Value == 64 && clockCyclesPerMicrosecond() == 20) // 3.1875 vs real value 3.2
      m=(m << 8) + t;
      return m + (m<<1) + ((m - (m>>2)) >> 2);
